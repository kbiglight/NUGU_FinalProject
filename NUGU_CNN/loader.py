...
# stock data loading
def load_stock_data(path, moving_window=128, columns=6, train_valid_ratio=8.0, valid_test_ratio = 1.0):
  ...
  def process_data(data):
    ...
    for idx in range(data.shape[0] - (moving_window + 1)):
      ...
      if data[idx+(moving_window+1),3] > data[idx+(moving_window),3]:
        lbl = [[1.0, 0.0]]
      else:
        lbl = [[0.0, 1.0]]
  ...
  # selecting 1/10 for testing, 1/10 for validating and 8/10 for training
  train_valid_idx = int((train_valid_ratio / 10) * labels_set.shape[0])
  valid_test_idx = int(((train_valid_ratio + valid_test_ratio) / 10) * labels_set.shape[0])
  train_stocks = stocks_set[:train_valid_idx,:,:]
  train_labels = labels_set[:train_valid_idx]
  print(len(train_stocks))
  valid_stocks = stocks_set[train_valid_idx:valid_test_idx,:,:]
  valid_labels = labels_set[train_valid_idx:valid_test_idx]
  print(len(valid_stocks))
  test_stocks = stocks_set[valid_test_idx:,:,:]
  test_labels = labels_set[valid_test_idx:]
  print(test_stocks[-5:])
  print(test_labels[-5:])
  train = DataSet(train_stocks, train_labels)
  valid = DataSet(valid_stocks, valid_labels)
