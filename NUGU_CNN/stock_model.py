...
class Model:
  ...
  def prediction_val(self):
    original_prediction = tf.argmax(self.prediction, 1)
    print(original_prediction)
    return original_prediction

  @define_scope
  def label_val(self):
    original_label = tf.argmax(self.label, 1)
...
def main():
  # Import data
  db = load_stock_data("data/kospi_with_news/")
  # random.seed(1234)
  # Construct graph
  ...
  with tf.Session(config=config) as sess:
    ...
    for i in range(1000):
      ...
      if i % 100 == 0:
        ...
        valid_images_eval, valid_labels_eval = db.validation.next_batch(100)
        val_acc = sess.run(model.accuracy, {image: valid_images_eval, label: valid_labels_eval, dropout: 1.0})
        accuracy = sess.run(model.accuracy, {image: images_eval, label: labels_eval, dropout: 1.0})
        print('step %d, val accuracy %g' % (i, val_acc))
        print('step %d, test accuracy %g' % (i, accuracy))
        if val_acc > val_score:
          best_model = model
          val_score = val_acc
    ...
    images_eval, labels_eval = db.test.next_batch(10000, shuffle=False)
    # best_model = saver.restore(sess, tf.train.latest_checkpoint('checkpoints/'))
    accuracy = sess.run(best_model.accuracy, {image: images_eval, label: labels_eval, dropout: 1.0})
    model_accuracy = sess.run(model.accuracy, {image: images_eval, label: labels_eval, dropout: 1.0})
    prediction_val = sess.run(best_model.prediction_val, {image: images_eval, label: labels_eval, dropout: 1.0})
    label_val = sess.run(best_model.label_val, {image: images_eval, label: labels_eval, dropout: 1.0})
    print(label_val)
    print(prediction_val)
    print('last model', model_accuracy)
