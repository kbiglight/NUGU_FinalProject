...
def preproc(data):
    ...
    data_news = normalize(data['News'].values)
    data_direction = data['Label']
    x = np.stack([data_open, data_close, data_high, data_low, data_volume, data_news])
...
def prepare_data(start_date, end_date, sequence_length):
    raw_data = pd.read_csv('kospi_with_news.csv')
...
train_sequence, positive_weight = prepare_data('2018-12-27', '2020-10-30', 5)
test_sequence, _ = prepare_data('2010-01-05', '2018-12-26', 5)
h_size = 32
model = RecurrentNet(6, h_size, 1)
extra_weight = 0
model, training_scores, testing_scores, iterations, losses = train_model(train_sequence, test_sequence, positive_weight + extra_weight, 0.0001, model, 300)
